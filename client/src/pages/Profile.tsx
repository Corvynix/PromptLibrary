...
export default function Profile() {
    ...
    const displayUsername = username && username !== "undefined" ? username : "Guest User";
    ...
    return (
        <LayoutGroup>
            <div className="min-h-screen bg-background text-foreground overflow-hidden relative flex flex-col p-4 md:p-6">
                ...
                <TechShell loading={loading} logoText="PROFILE">
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                        <div className="max-w-5xl mx-auto space-y-8 pb-20 pt-8 px-2 md:px-0">
                            {/* Profile Header */}
                            <div className="relative">
                                {/* Banner */}
                                <div className="h-48 md:h-64 border-2 border-foreground/20 rounded-3xl bg-muted overflow-hidden relative">
                                ...
                                <div className="px-6 md:px-10 relative -mt-16 flex flex-col md:flex-row items-end md:items-center justify-between gap-6">
                                    <div className="flex items-end gap-6">
                                        <div className="relative group">
                                            <div className="w-32 h-32 border-4 border-background rounded-3xl bg-background p-1 shadow-xl">
                                            ...
                                            <div className="absolute -bottom-2 -right-2 bg-background border border-foreground/20 p-1 rounded-full">
                                                <div className="w-6 h-6 bg-yellow-400 rounded-full flex items-center justify-center text-xs font-black text-black shadow-sm" title="Level 42">
                                                    42
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="mb-2">
                                            <h1 className="text-3xl font-black tracking-tighter uppercase flex items-center gap-2">
                                                {displayUsername}
                                                <Badge variant="secondary" className="bg-blue-500/10 text-blue-500 border-blue-500/20 rounded-full">
                                                    PRO
                                                </Badge>
                                            </h1>
                                            ...
                                            <div className="flex items-center gap-4 mt-4 text-xs font-mono text-muted-foreground">
                                                ...
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <a href="#" className="hover:text-foreground transition-colors">PROMPTWIZARD.IO</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-3 mb-2">
                                        <Button variant="outline" size="icon" className="rounded-full w-10 h-10 border-foreground/20">
                                        ...
                                        <Button
                                            ...
                                            className={cn(
                                                "rounded-full min-w-[120px] transition-all font-bold tracking-wider",
                                                isFollowing
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 px-2">
                                ...
                                <div className="space-y-6">
                                    <Card className="rounded-3xl border-2 border-foreground/10 bg-card/50 backdrop-blur-sm">
                                        <CardContent className="p-6 grid grid-cols-2 gap-4">
                                            <div className="text-center p-4 border border-foreground/10 rounded-2xl bg-background/50">
                                            ...
                                            <div className="text-center p-4 border border-orange-500/20 rounded-2xl bg-orange-500/5">
                                        ...
                                        <CardContent>
                                            <div className="grid grid-cols-3 gap-2">
                                                {ACHIEVEMENTS.map((achievement) => (
                                                    <div key={achievement.id} className="aspect-square border border-foreground/10 rounded-xl bg-background/50 flex flex-col items-center justify-center p-2 gap-2 hover:border-foreground/30 transition-colors cursor-pointer group relative">
                                                        <div className="text-2xl group-hover:scale-110 transition-transform">{achievement.icon}</div>
                                                        <div className="text-[8px] text-center font-bold tracking-wider opacity-0 group-hover:opacity-100 absolute bottom-1 transition-opacity uppercase text-foreground">
                                                            {achievement.name}
                                                        </div>
                                                    </div>
                                                ))}
                                                {Array.from({ length: 6 }).map((_, i) => (
                                                    <div key={i} className="aspect-square border border-foreground/5 rounded-xl bg-background/30 flex items-center justify-center opacity-30">
                                ...
                                <div className="lg:col-span-2">
                                    <Tabs defaultValue="prompts" className="w-full">
                                        <TabsList className="w-full justify-start h-12 bg-transparent border-b-2 border-foreground/10 rounded-none p-0 mb-6">
                                            <TabsTrigger
                                                value="prompts"
                                                className="h-12 rounded-none border-b-2 border-transparent data-[state=active]:border-blue-500 data-[state=active]:bg-transparent data-[state=active]:text-blue-500 px-6 font-bold tracking-widest text-xs"
                                            >
                                                PROMPTS
                                            </TabsTrigger>
                                            <TabsTrigger
